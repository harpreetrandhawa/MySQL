...
where status = 'exportable' 
 and date(occured_at + interval 4 hour) >= date_format((curdate()),"%Y-%m-01") group by 1,2)exp_date
...
left join
    ...
    from  seller_centre_ae_replica.sales_order_item_status_history 
...
and exp_date.occured_at is not null and wadi_indexer.vendor_warehouse_country not in  ('CN','HK','AF') and wadi_indexer.category_level_1 != 'daily_needs') as main;
