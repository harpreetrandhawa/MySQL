select distinct concat(UCASE(ind.db),ind.id_sales_order_item)as item_id,order_status.order_nr,
   ...
   order_status.po_created_at as po_created_at,
    CASE
   ...
   END AS Status,
    CASE
      ...
      WHEN erp.arrivel_datetime = 'No' AND (erp.po_number <> 'No' or order_status.po_number <>'') AND erp.pick_registered = 'No' AND TIMESTAMPDIFF(MINUTE,ifnull(order_status.exportable_at,order_status.exported_at),DATE_ADD(now(),INTERVAL 4 HOUR)) > 2160 THEN 'Handover priority 1'
      WHEN erp.arrivel_datetime = 'No' AND (erp.po_number <> 'No' or order_status.po_number <>'') AND erp.pick_registered = 'No' AND TIMESTAMPDIFF(MINUTE,ifnull(order_status.exportable_at,order_status.exported_at),DATE_ADD(now(),INTERVAL 4 HOUR)) BETWEEN 1440 AND 2160 THEN 'Handover priority 2'
      WHEN erp.arrivel_datetime = 'No' AND (erp.po_number <> 'No' or order_status.po_number <>'') AND erp.pick_registered = 'No' AND TIMESTAMPDIFF(MINUTE,ifnull(order_status.exportable_at,order_status.exported_at),DATE_ADD(now(),INTERVAL 4 HOUR)) <= 1440 THEN 'Handover priority 3'
...
CASE
   ...
   END as rank,
    erp.arrivel_datetime as arrived_at,
  ...
  wc.category,
  order_status.arrived_at as arrived_date,wc.brand
from nm_sourcing.sales_order_item_custom as ind
   ...
   nm_sourcing.am_vendor_mapping as owner on owner.vendor_name = erp.vendor_code or owner.vendor_name = order_status.vendor_code
    LEFT OUTER JOIN
   (select bids,product_name,sup_category,size,category,brand from wadi_catalog) as wc on wc.bids  = ind.bids
    LEFT OUTER JOIN
   (select * from nm_sourcing.vendor_ranking where in_stock !=0 order by rank ) as first_vendor  on ind.bids = first_vendor.wadi_sku_num
    LEFT outer JOIN
   (select * from nm_sourcing.vendor_ranking where in_stock !=0 and vendor_location = 'UAE' order by uae_rank) as uae_vendor
    on ind.bids = uae_vendor.wadi_sku_num
    LEFT outer JOIN
   (select * from nm_sourcing.vendor_ranking where in_stock !=0 and vendor_location = 'KSA' order by ksa_rank) as ksa_vendor
    on ind.bids = ksa_vendor.wadi_sku_num
where ind.status in ('exported','exportable','carrier_selection_pending') and erp.pick_line <> 'Yes' having first_vendor not in ('SV00596','SV00602','SV00601','SV00600','SV00599','SV00598','SV00597','SV00595','SV00646')
Limit 5