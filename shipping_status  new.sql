...
brand,
vendor_code,
wadi_indexer.exported_at exportable_at
...
-- left join
-- (select db,item_id,min(occured_at)occured_at 
-- from status_cache 
-- where status = 'exportable' 
--  and date(occured_at) >= date_format((curdate() - interval 1 month),"%Y-%m-01") group by 1,2)exp_date
...
and date(exported_at) >= date_format((curdate() - interval 1 month),"%Y-%m-01") and wadi_indexer.vendor_warehouse_country not in  ('CN','HK','AF') and wadi_indexer.vendor_code not in ('1290','6797','6818')
 and wadi_indexer.category_level_1 not in ( 'daily_needs','seller_accessories')) as main;