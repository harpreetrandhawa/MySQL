...
(select 
awb
,min(if(reference ='Aramex-SH014',occurred_at,null)) as pickup
from logistics.shipment_update 
where occurred_at > curdate() - interval 90 day
group by 1)as su on su.awb = buy_sell.wadi_indexer.HAWB_number

where
soi.fk_seller not in (1, 11) and soi.fk_shipment_type = '2'
and soi.fk_sales_order_item_status in (1,2,8)
