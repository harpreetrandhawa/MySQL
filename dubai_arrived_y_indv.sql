...
FROM 
(select item_id,(exportable_at + interval 4 hour)exported_at,(if(arrived_at>shipped_at,shipped_at,arrived_at)+ interval 4 hour)arrived_at,
...
where (if(arrived_at>shipped_at,shipped_at,arrived_at) + interval 4 hour) between date_format(curdate()- interval 1 day,'%Y-%m-%d 08:00:00') and date_format(curdate(),'%Y-%m-%d 08:00:00')
and canceled_at = 0 and floor((dayofweek(date(exportable_at + interval 4 hour)-interval 6 day)+timestampdiff(day,date(exportable_at + interval 4 hour),Date(if(arrived_at>shipped_at,shipped_at,arrived_at) + interval 4 hour)))/7) < 100
