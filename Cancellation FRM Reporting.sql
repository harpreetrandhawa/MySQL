SELECT
    ...
    t.transfer_price,    if(t.owner='NULL','Not Assigned',t.owner) as owner,    t.max_shipping_date,t.demo,t.po_created_at
FROM
    (SELECT
	...
	,wadi_indexer.sku
	,wadi_indexer.bids
    ,if(mid(wadi_indexer.sku,6,2)='NM','Namshi','Wadi') as type
    ,wadi_indexer.exported_at as ExportableDateTime
    ,wadi_indexer.po_created_at
	...
	END,'Find Seller') as transfer_price 
	,ifnull(CASE
	...
	,date_add(date(date_add(wadi_indexer.exported_at,interval 4 hour)),interval if(wadi_indexer.db = 'SA',seller_shipping_time_max_sa,seller_shipping_time_max_ae) day) as shipping_date
	
	,date_add(date(date_add(date(date_add(wadi_indexer.exported_at,interval 4 hour)),interval if(wadi_indexer.db = 'SA',seller_shipping_time_max_sa,seller_shipping_time_max_ae) day)), interval (floor((if(wadi_indexer.db = 'SA',wadi_indexer.seller_shipping_time_max_sa,wadi_indexer.seller_shipping_time_max_ae)+ dayofweek(date(date(date_add(wadi_indexer.exported_at, interval 4 hour))-interval 6 day)))/7)) + 1 day) as max_shipping_date
	
    ,wadi_indexer.vendor_code as demo
FROM
...
WHERE
	wadi_indexer.bob_item_status in ('exported', 'exportable')) t
WHERE t.status in('TP Confirmation','PO Created','Arrived','DXB Inventory pickup','KSA Inventory pickup')
AND t.max_shipping_date <= CURDATE() AND t.bids != 'W1315999999'