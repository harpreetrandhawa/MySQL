...
and canceled_at = 0 and 
floor((dayofweek(date(exportable_at + interval 4 hour)-interval 6 day)+timestampdiff(day,date(exportable_at + interval 4 hour),Date(if(arrived_at>shipped_at,shipped_at,arrived_at) + interval 4 hour)))/7) <100
Having vendor = 'Saudi')t group by 1 having owner is not null;
