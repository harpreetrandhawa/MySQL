...
SELECT
	...
	,CASE
		...
		CASE
            ...
            WHEN restricted.bids IS NULL AND indexer.db = 'SA'
		    THEN IFNULL(REPLACE(REPLACE(REPLACE(procurement_owner.owner,',',''),'\n',''),'\r',''),'Saudi')
...
WHERE
  ...
  )t where t.Status  in ('TP Confirmation','PO Created')